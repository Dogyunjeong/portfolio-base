...
export default class TemplateService {
  ...
  private _findPageContent(
    pageContent: TemplateTypes.PageContent,
    pagePath?: string
  ) {
    if (_.isNil(pageContent.pages)) return null;
  ...
  private _getPageInfo(
    ...
    TemplateService.pageConfigProperty.forEach(
      (property: TemplateTypes.PageConfigProperty) => {
        if (pageConfig[property]) {
          const content = this._findPageContent(
            pageContent,
            pageConfig[property]
          );
  ...
  public static async getPageList(): Promise<TemplateTypes.PageList> {
    ...
    const makePageList = (
      pageContent: TemplateTypes.PageContent,
      list: TemplateTypes.PageList
    ) => {
      list[pageContent.path] = {};
      if (pageContent.pages && pageContent.pages.length > 0) {
        pageContent.pages.forEach((page) =>
          makePageList(page, list[pageContent.path])
        );
      }
      return list;
    };
  ...
  public static async getPageDetail(
    ...
    const {
      ...
      pageContent: TemplateTypes.PageContent;
      layout: CustomLayoutTypes.Layout;
    } = instance._getPageInfo(pageConfig);
    const components: CustomComponentTypes.CustomComponentBase[] =
  ...
  public static async addPage(baseUrl: string, path: string): Promise<void> {
    const pageConfig: TemplateTypes.PageConfig = TemplateService.urlToPageConfig(
      baseUrl
    );
    const instance = await TemplateService.getInstance();
    const {
      pageContent,
    }: { pageContent: TemplateTypes.PageContent } = instance._getPageInfo(
      pageConfig
    );
